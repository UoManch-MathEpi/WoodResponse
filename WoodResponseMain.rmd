...
repTS <- 10 # Reporting time step, i.e. time steps in numerical solution between observed data points
...
enddate <-  as.Date(offset+duration)
    ...
    Sim <- seir_tau_tv(MaxTime=duration*repTS,dt=1/repTS, repro=R0,sig=1/LatP, gam= 1/InfP, N0=pop,
  ...
  for(i in c(2,4,8,12,16,20,24,32,48, floor(dim(Sim)[1]/repTS)-120)){  
    interventdate <- 120
